...
public:
	...
	{
		pixel = pixelll; 
		dir.izmena(pixel.x-camera.x,pixel.y-camera.y,pixel.z-camera.z);
		dir = p.Normalize(dir);
	}

	point tracing (int number, int n, point ray, point camera, point lamp)
	{
		long double crossfire, check_crossfire; 
		...
		Material(objects, find2.number); 

		lambert = Cosinus(dir_light, objects[find2.number].normal); 
...
void RenderScene(void)
{
	...
	point lamp(550.0,600.0,400.0); 
	point camera(220.0,500.0,600.0); 
	point color;
